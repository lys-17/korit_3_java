 ...
 */
        ...
        double calculateAvg(double totalScore, int numOfSubs) {
            double avgScore = totalScore / numOfSubs;
            return avgScore;
        }
...
//        System.out.println(review06.calculateAvg(sum, subs));
        ...
        /*
            여태까지 메서드 정의한 것의 문제점은 두 과목의 점수를 더란 값만 반환한다는 의미였음
        ...
        double avgScore = 0;
        int subjects = 0;
        // for문으로 반복문을 작성한다고 가정할 경우에 한계값이 사전에 지정 되어야함으로 과목수를 먼저 물어보는 출력문 작성
        System.out.println(" 몇 과목 입력하시겠습니까? >>>");
        ...
        for(int i = 0; i < subjects; i++) {
            ...
            totalScore = totalScore + scanner.nextDouble();
        }
        avgScore = review06.calculateAvg(totalScore, subjects);
